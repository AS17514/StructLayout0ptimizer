...
async function optimizeStruct() {
    ...
    const structInfo = (0, structAnalyzer_1.findEnclosingStruct)(text, offset);
    if (!structInfo || structInfo.fields.length < 2) {
        vscode.window.showInformationMessage(i18n("no_struct"));
        return;
    }
    const result = (0, layoutCalculator_1.computeOptimalOrder)(structInfo.fields, 0);
    ...
    edit.replace(document.uri, new vscode.Range(document.positionAt(bodyStart), document.positionAt(bodyEnd)), newBody);
    await vscode.workspace.applyEdit(edit);
    // Show notification
    const pct = result.currentLayout.totalSize > 0
        ? Math.floor((result.bytesSaved * 100) / result.currentLayout.totalSize)
        : 0;
    const msg = i18n("optimized_summary", String(result.bytesSaved)) +
        "\n" +
        i18n("optimized_item", String(result.currentLayout.totalSize), String(result.optimalLayout.totalSize), String(pct));
...
function activate(context) {
    context.subscriptions.push(vscode.commands.registerCommand("struct-layout-optimizer.optimize", optimizeStruct), vscode.commands.registerCommand("struct-layout-optimizer.toggleLanguage", toggleLanguage));
}
function deactivate() { }
